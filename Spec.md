...
- **What this deliberately doesn't do:** no per-staff-member accounts, no password reset flow (an admin resets it via the edit-tenant form), no write actions from this panel (view-only — cancelling/rescheduling still only happens through the WhatsApp flow itself).
