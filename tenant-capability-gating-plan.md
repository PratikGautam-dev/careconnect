...
**Status: done.** Backend capability gating (schema, repository, `portal/capabilities.py`,
`require_capability`, doctor/department route guards, onboarding/tenant-edit API support)
implemented and covered by `backend/tests/test_capability_gating.py` (17 tests). Follow-up
bot-flow/UI work also complete: "Back" extended into `flows/patient_identity.py`'s
name/age/duplicate-decision/relationship states; clinic bookings auto-skip department+doctor
selection when a tenant has exactly one of each (`flows/booking/book.py`, covered by
`backend/tests/test_clinic_single_doctor_booking.py`); the onboarding wizard, portal sidebar,
and admin tenant-edit page were updated on the frontend to match (clinic-simplified onboarding
form, hidden "Doctors" nav for clinics, tenant type/capabilities editor). Full backend suite green.
