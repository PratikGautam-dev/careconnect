...
"""Google Meet integration (Spec.md Section 0): storage for one doctor's
optional Google Calendar connection. A row's mere EXISTENCE means "this
doctor is connected" -- unlike db/repositories/hospital_settings.py's lazy
blank-row pattern, no row is ever created except by a successful OAuth
...
def get_calendar_connection(doctor_id: str) -> dict | None:
    session = get_session()
    row = session.execute(
        select(GoogleCalendarConnection).where(GoogleCalendarConnection.doctor_id == doctor_id)
    ).scalar_one_or_none()
    return _row_to_dict(row) if row else None


def upsert_calendar_connection(
    doctor_id: str, hospital_id: int, google_email: str | None,
    encrypted_access_token: str, encrypted_refresh_token: str, access_token_expires_at: str,
    calendar_id: str = "primary",
) -> dict:
    """The OAuth callback's own write -- a fresh connect, or a doctor
    reconnecting/re-consenting overwrites their existing row entirely
    ...
    session.execute(
        ...
        .values(
            doctor_id=doctor_id, hospital_id=hospital_id, google_email=google_email,
    ...
    session.commit()
    return get_calendar_connection(doctor_id)


def update_calendar_access_token(doctor_id: str, encrypted_access_token: str, access_token_expires_at: str) -> None:
    """Called after a refresh-token round trip (modules/google_calendar.py)
    for an ALREADY-connected doctor -- only the access token/expiry actually
    change; the refresh token itself is long-lived and untouched here. A
    plain UPDATE (not an upsert): this is never the first write for a
    doctor, so there is nothing to insert a fallback row for."""
    session = get_session()
    session.execute(
        update(GoogleCalendarConnection)
        .where(GoogleCalendarConnection.doctor_id == doctor_id)
...
def delete_calendar_connection(doctor_id: str) -> bool:
    session = get_session()
    result = session.execute(delete(GoogleCalendarConnection).where(GoogleCalendarConnection.doctor_id == doctor_id))
    session.commit()
