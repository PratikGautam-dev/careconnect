...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-10 w-px whitespace-nowrap px-space-2 text-left align-middle text-[11.5px] font-semibold text-ink-400 uppercase first:pl-0 [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
...
function TableCell({ className, ...props }: React.ComponentProps<"td">) {
  return (
    <td
      ...
      className={cn(
        "w-px whitespace-nowrap px-space-2 py-space-2 align-middle first:pl-0 [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
