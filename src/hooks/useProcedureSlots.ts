...
// The procedure-scoped sibling of useTestSlots -- same manual per-slot
// override shape, pointed at /api/portal/procedures/{procedureId}/slots...
// instead of /api/portal/diagnostic-tests/....
export function useProcedureSlots(procedureId: number) {
  ...
  const { data: slots, refetch } = useQuery({
    queryKey: ["portal-procedure-slots", procedureId, viewAll, date],
    retry: false,
    queryFn: async () => {
      const qs = viewAll ? "" : `?date=${date}`;
      const result = await portalFetch(`/api/portal/procedures/${procedureId}/slots${qs}`);
  ...
  const toggleBlockMutation = useMutation({
    mutationFn: async ({ scheduledAt, blocked }: { scheduledAt: string; blocked: boolean }) => {
      const result = await portalFetch(`/api/portal/procedures/${procedureId}/slots/block`, {
        method: "POST",
  ...
  const removeSlotMutation = useMutation({
    mutationFn: async (scheduledAt: string) => {
      const result = await portalFetch(`/api/portal/procedures/${procedureId}/slots/remove`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const addSlotMutation = useMutation({
    mutationFn: async ({ addDate, time }: { addDate: string; time: string }) => {
      const result = await portalFetch(`/api/portal/procedures/${procedureId}/slots/add`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
