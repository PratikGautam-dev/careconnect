...
export function ActivityFeed({ items }: { items: ActivityItem[] }) {
  return (
    <Card className="p-space-4">
      <h3 className="text-label mb-space-3 text-ink-900 font-bold">Recent activity</h3>
      {items.length === 0 ? (
        <p className="py-space-4 text-ink-400 text-center text-[13px]">Nothing has happened yet.</p>
      ) : (
        <ul className="space-y-space-3">
          {items.map((item, i) => {
            const Icon = ICONS[item.label] || CalendarCheck;
            return (
              <li key={i} className="gap-space-3 flex items-start">
                <div className="bg-brand-50 text-brand-600 mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full">
                  <Icon size={13} strokeWidth={2} />
                </div>
                <div className="min-w-0 flex-1 text-[12.5px]">
                  <p className="text-ink-900">
                    <span className="font-semibold">{item.phone}</span> — {item.label.toLowerCase()}{" "}
                    with {item.doctor_name} ({item.department_name})
                  </p>
