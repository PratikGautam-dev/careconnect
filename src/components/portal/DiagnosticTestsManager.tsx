  ...
  return (
    <Card className="p-space-4">
      <div className="mb-space-3 gap-space-2 flex flex-wrap items-center justify-between">
        <h3 className="text-label text-ink-900 font-bold">Tests</h3>
        <div className="gap-space-1 bg-paper p-space-1 flex rounded-md">
          {CATEGORIES.map((c) => (
            <button
              key={c.id}
              type="button"
              ...
              className={cn(
                "px-space-3 py-space-1 rounded-md text-[12.5px] font-semibold transition-colors duration-150",
                category === c.id
      ...
      <p className="mb-space-3 text-ink-400 text-[12px]">
        Each test has its own price and weekly schedule -- the schedule below determines the
                ...
                {isEditing ? (
                  <div className="border-line bg-paper p-space-4 rounded-lg border">
                    <div className="mb-space-3 flex items-center justify-between">
                      <p className="text-label text-ink-900 font-semibold">Edit test</p>
                      <button
                        type="button"
                    ...
                    <ScheduleFields
                      form={editTestForm}
                      setForm={setEditTestForm}
                      toggleDay={toggleEditTestDay}
                      namePrefix="edit_test"
                    />
                    <div className="gap-space-2 flex justify-end">
                      <Button variant="secondary" onClick={() => setEditingTestId(null)}>
                        Cancel
                      </Button>
                      <Button
                        onClick={() => saveEditTest(test.id)}
                        disabled={pendingKey === `test-${test.id}` || !editTestForm.name.trim()}
                      >
                  ...
                  <div className="gap-space-2 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <div className="gap-space-3 flex items-center">
                      ...
                      <Badge tone={test.is_active ? "success" : "neutral"}>
                      ...
                      {canManage && (
                        <button
                          ...
                          disabled={pendingKey === `test-${test.id}`}
      ...
      {canManage &&
        (showAddTest ? (
          <div className="border-line bg-paper p-space-4 rounded-lg border">
            <div className="mb-space-3 flex items-center justify-between">
              <p className="text-label text-ink-900 font-semibold">
                Add {category === "diagnostic" ? "Diagnostic Test" : "Lab Test"}
              </p>
              <button
                type="button"
            ...
            <ScheduleFields
              form={newTestForm}
              setForm={setNewTestForm}
              toggleDay={toggleNewTestDay}
              namePrefix="new_test"
            />
            <div className="gap-space-2 flex justify-end">
              <Button variant="secondary" onClick={() => setShowAddTest(false)}>
                Cancel
              </Button>
              <Button onClick={handleAddTest} disabled={savingTest || !newTestForm.name.trim()}>
