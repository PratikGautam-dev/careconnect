...
Same "real backend, graceful local fallback" shape as core/history.py's
...
already uses) verified by the streaming endpoint in portal_api.py.
"""
import hashlib
import hmac
import os
...
_LOCAL_STORAGE_DIR = Path(os.environ.get("LOCAL_STORAGE_DIR", "local_storage"))
...
class LocalFileStorage:
    ...
    presigned URL. portal_api.py's document-download route verifies the
    ...
    if bucket:
        try:
            return S3Storage(
                ...
                endpoint_url=os.environ.get("S3_ENDPOINT_URL"),  # set for R2/B2, unset for real AWS S3
                access_key_id=os.environ.get("S3_ACCESS_KEY_ID"),
                secret_access_key=os.environ.get("S3_SECRET_ACCESS_KEY"),
            )
        except Exception:
            pass
    secret = os.environ.get("PORTAL_SECRET", "dev-only-local-storage-secret")
