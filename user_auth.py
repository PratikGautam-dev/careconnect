...
gets. Deliberately does NOT replace portal.py/portal_api.py's existing
...
Delivered to the frontend the same way portal_api.py's token is: a Bearer
...
cookie problem portal_api.py's own module docstring already worked through.
...
from portal import _SESSION_TTL_SECONDS, _sign_session
...
# Deliberately shorter-lived than portal.py's own 24h hospital session --
...
def _verify_user_session(token: str) -> int | None:
    ...
    portal.py's _verify_session(), just keyed to user_id + AUTH_SECRET."""
    if not token:
        return None
    parts = token.split(".")
    if len(parts) != 3:
...
async def auth_select_hospital(payload: dict, authorization: str | None = Header(default=None)):
    ...
    portal_api.py's password login issues (_sign_session) -- the frontend
