...
Covers:
  ...
  - portal_api.py's new /api/portal/patients/{id}* routes end to end,
...
def _isolated_local_storage(tmp_path):
    ...
    reason core/history.py's session store is one) -- tests need their own
...
def fake_whatsapp_document_send(monkeypatch):
    ...
    monkeypatch.setattr(portal_api.WhatsAppClient, "send_document", fake_send_document)
...
# --- portal_api.py: patient detail route ---
...
# --- portal_api.py: send to WhatsApp ---
...
def test_send_document_failure_does_not_crash_and_shows_error(two_hospitals, patient_a, monkeypatch):
    ...
    monkeypatch.setattr(portal_api.WhatsAppClient, "send_document", failing_send_document)
