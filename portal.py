...
(frontend/src/app/portal/*, backed by portal_api.py's JSON endpoints) does
all of it -- removed. What's left is the session-signing logic
portal_api.py and user_auth.py both still import directly (a Bearer-token
...
register, so core/main.py no longer includes one for this module.
"""
import hashlib
import hmac
import os
...
def _verify_session(cookie_value: str) -> int | None:
    ...
    now, not a cookie -- see portal_api.py's module docstring for why."""
    if not cookie_value:
        return None
...
def _build_new_booking_context(hospital) -> tuple[list[dict], dict, dict]:
    """Shared by portal_api.py's new-booking GET (blank form) and POST
